...
# $Id: vtables.pod 9344 2005-10-05 03:02:04Z jhoblitt $
...
F<classes/> subdirectory; this is where all the built-in PMC classes live. (And
a good source of examples to plunder even if you're not writing a core data
type.)

You should almost always start by running F<classes/genclass.pl> to generate a
...
F<build_tools/pmc2c.pl> program) with stubs for all the methods you need to
...
declaration in F<classes/YOURCLASS.pmc>. Then, change the type of the C<init>
...
Add classes/YOURCLASS.pmc to the MANIFEST
...
The usual way to continue from the F<genclass.pl>-generated skeleton is to
define a structure that will hook onto the C<data>, if your data type needs to
use that, and then also define some user-defined flags.
...
The master list of vtable methods can be found in F<vtable.tbl> in the root
...
classes/YOURCLASS.pmc, add the C<extends SUPERCLASS> phrase. For example:

  pmclass PackedArray extends Array { ...

See the POD documentation in F<build_tools/pmc2c.pl> for a list of useful
