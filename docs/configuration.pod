...
# $Id: configuration.pod 10333 2005-12-04 00:04:48Z jhoblitt $
...
All steps are really modules; they should start with a declaration setting the
current package to L<Configure::Step>. Note that there is no file for the
L<Configure::Step> package.

XXX: L<Configure::Step> will be going away in favor of individual namespaces
I<RSN>.
...
specifically told to. To do this, edit the L<Parrot::Configure::RunSteps>
module's C<@steps> array. Steps are run in the sequence in which they appear in
C<@steps>.
...
the L<Parrot::Configure::Data> package.   You get and set configuration
system's data by calling methods on this package. The methods are listed below.

=over 4

=item C<< Parrot::Configure::Data->get(keys) >>

Returns the values for the given keys.

=item C<< Parrot::Configure::Data->set(key, value, [key, value, ...]) >>

Sets the given keys to the given values.

=item C<< Parrot::Configure::Data->add(delim, key, value, [key, value, ...]) >>

Sets the given keys to the given values or appends values delimited by I<delim>
to existing keys.

=item C<< Parrot::Configure::Data->keys() >>

Returns a list of all keys.

=item C<< Parrot::Configure::Data->dump() >>
