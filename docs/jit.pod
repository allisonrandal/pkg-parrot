...
# $Id: jit.pod 9344 2005-10-05 03:02:04Z jhoblitt $
...
B<FLOAT_REGISTERS_TO_MAP> as nonzero, this amount of most used registers per
code section are mapped to native processor registers.
...
This macro, when implemented, should return the register number of the register
base pointer. This is currently the C<interpreter> itself, but will change soon
into a distinct frame pointer.

Additionally, if this macro is implemented, the register preserving code is
calling the register save functions with the C<_offs> suffix. These functions
get the offsets of the registers as arguments.
...
To obtain register offsets, another set of macros exists, that have C<OFFS> in
their names:
